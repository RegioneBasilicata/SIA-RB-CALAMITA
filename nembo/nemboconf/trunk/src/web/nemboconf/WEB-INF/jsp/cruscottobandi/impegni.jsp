		...
		<div class="row">
			<div class="moduletable">
				<ul class="breadcrumb">
					...
					<li class="active">Inserisci (Impegni) </li>
				</ul>
			</div> 
		</div>
	</div>
	<p:messaggistica/>
	<p:CruscottoBandiHeader activeTab="IMPEGNI" cu="CU-NEMBO-015-P"></p:CruscottoBandiHeader>
		...
		<c:if test="${msgErrore != null}">
			...
			<form:form action="" modelAttribute="" id="mainForm" method="post" class="form-horizontal" style="margin-top:2em">
				...
				<div class="form-group">
				    <label for="tipologia" data-toggle="modal" data-target="#gruppiModal" class="col-sm-4 control-label">
				 ...
				 <p>Nel testo degli impegni  è possibile aggiungere dei campi variabili che dovranno essere inseriti in fase di compilazione della dichiarazione: $$STRING (campo stringa di libera digitazione, accetta qlc carattere); $$NUMBER (numero che può essere anche inserito con valori decimali usando la virgola come carattere separatore); $$INTEGER (valore numerico intero); $$DATE (campo in formato data gg/mm/aaaa).</p>
				 
				 <c:if test="${elencoGruppiInfo != null || nuoveDichiarazioni != null}">
				 	<fieldset>
				 	<legend>Impegni importati da catalogo</legend>
					 <table id="tGruppo" class="table table-hover table-striped table-bordered tableBlueTh" >
						...
						<thead>
							<tr> 
								...
								<th>Titolo impegno e Dettaglio impegno</th>
								<th>Obbligatorio</th> 
							</tr>
						</thead>
						<tbody>
					...
					<fieldset>
				 		<legend>Impegni aggiuntivi</legend>
						...
						<colgroup>
							...
							<thead>
								<tr> 
									...
									<th>Titolo impegni e Dettaglio impegni</th>
									<th>Obbligatorio</th> 
									<th>Vincolo tra gli impegn</th>
									<th>Ordine</th>
								</tr>
							</thead>
							<tbody>
								<c:choose>
									<c:when test="${nuoveDichiarazioni != null}">
										<c:forEach items="${nuoveDichiarazioni}" var="a" varStatus="l">
											<tr class="newGroup row_${l.index}"><td colspan="5"><input type="hidden" id="idGruppoInfo_${l.index}" name="idGruppoInfo_${l.index}" value="${a.idGruppoInfo}"><i>Titolo impegno *</i></td></tr>
									...
									<c:when test="${elencoGruppiInfoNoCatalogo != null }">
										<c:forEach items="${elencoGruppiInfoNoCatalogo}" var="a" varStatus="l">
											<c:if test="${a.flagInfoCatalogo == 'N'}">
												...
												<tr class="newGroup newDetail row_${l.index} detRow_${l.index} "><td colspan="5"><i>Dettaglio impegno * <c:if test="${flagAttivoDichiarazione == 'N'}"><a onclick="addNewDettaglioInfo(${l.index});return false;" href="#" class="ico16 ico_add" style="margin-left:1em"></a><a style="margin-left:1em" onclick="reorderManager(${l.index});return false;" href="#" class="ico16 ico_up_down"></a></c:if></i></td></tr>
												<c:forEach items="${a.elencoDettagliInfo}" var="b" varStatus="c">
													<tr class="newGroup newDetail row_${l.index} detRow_${l.index} detRow_${c.index}_${l.index}">
														<td>
															<input type="hidden" id="idDettaglioInfo_${c.index}_${l.index}" name="idDettaglioInfo_${c.index}_${l.index}" value="${b.idDettaglioInfo}">
				 ...
				 <div class="form-group puls-group" style="margin-top: 2em">
					<div class="col-sm-12">
						<button type="button" onclick="forwardToPage('dichiarazioni.do');" class="btn btn-default">indietro</button>
			    ...
			    <div class="modal fade" id="gruppiCatalogoModal" tabindex="-1" role="dialog" aria-labelledby="gruppiCatalogoModalLabel" aria-hidden="true">  		
					<div class="modal-dialog" style="width: 850px">                                                                                                         			
					  <div class="modal-content">                                                                                                      			
					    <div class="modal-header">                                                                                                     			
					      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> 	
					      <h4 class="modal-title" id="myModalLabel">Catalogo impegni Standard</h4>                                                               		
					    </div>                                                                                                                         				
					    <div class="modal-body">   
							<table id="tGruppo" class="table table-hover table-striped table-bordered tableBlueTh" >
								<colgroup>
							...
							<table id="tVincoli" class="table table-hover table-striped table-bordered tableBlueTh" >
								...
								<tbody>
									<tr class="hRowVincoli" style="display:none">
									 	...
									 	<td><m:select  viewTitle="true" viewOptionsTitle="true" id="tipiVincoli_$$index" preferRequestValues="${preferRqParameter}" onchange="$('#idVincolo_$$index').val($('#tipiVincoli_$$index option:selected').text()+'_'+$$index);" list="${tipiVincoliImp}" name="tipiVincoli_$$index" valueProperty="id" textProperty="descrizione"></m:select></td>
									...
									<c:if test="${vincoliTrovati != null}">
										<c:forEach items="${vincoliTrovati}" var="a" varStatus="idx">
											<tr class="rowVincoli"  data-index="${idx.index}"> 
										 		<td><a onclick="$(this).closest('tr').remove();return false;" class="ico24 ico_trash" href="elimina_vincolo.do"></a></td>
										 		<td><m:select  viewTitle="true" viewOptionsTitle="true" selectedValue="${a.codice}" id="tipiVincoli_${idx.index}" preferRequestValues="${preferRqParameter}" onchange="$('#idVincolo_${idx.index}').val($('#tipiVincoli_${idx.index} option:selected').text()+'_${idx.index});" list="${tipiVincoliImp}" name="tipiVincoli_${idx.index}" valueProperty="id" textProperty="descrizione"></m:select></td>
...
<script type="text/javascript">
	...
	function confermaGruppo()
	{
		...
		$('#mainForm').attr('action','visualizzaimpegni.do').submit();
	}
	function confermaGruppoCatalogo()
	{
		$('#gruppiCatalogoModal').modal('hide');
		$('#mainForm').attr('action','refreshGruppoInfoImpegni.do').submit();
	}
	...
	function submitConferma()
	{
		...
		$('#mainForm').attr("action", "confermaimpegni.do");
		$('#mainForm').submit();
	...
	{
		var act = $('#mainForm').attr("action");
		$('#mainForm').attr("action", "avantiimpegni.do");
		$('#mainForm').submit();
		$('#mainForm').attr("action", act);
	}
</script>	
