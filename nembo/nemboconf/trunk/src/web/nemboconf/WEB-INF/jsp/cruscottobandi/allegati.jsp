		...
		<div class="row">
			<div class="moduletable">
				<ul class="breadcrumb">
					...
					<li class="active">Inserisci (Allegati) </li>
				</ul>
			</div> 
		</div>
	</div>
	<p:messaggistica/>
	<p:CruscottoBandiHeader activeTab="ALLEGATI" cu="CU-NEMBO-015-A"></p:CruscottoBandiHeader>
			...
			<form:form action="" modelAttribute="" id="mainForm" method="post" class="form-horizontal" style="margin-top:2em">
				...
				<div class="form-group">
				 ...
				 <c:if test="${elencoGruppiInfo != null ||  nuoveDichiarazioni!= null}">
				 	<fieldset>
				 	<legend>Allegati importati da catalogo</legend>
					 <table id="tGruppo" class="table table-hover table-striped table-bordered tableBlueTh" >
						...
						<thead>
							<tr> 
								...
								<th>Titolo allegato e dettaglio allegato</th>
								<th>Obbligatorio</th> 
								<th>Tipo File</th>  
							</tr>
						</thead>
						<tbody>
						  <c:forEach items="${elencoGruppiInfoCatalogo}" var="e" varStatus="st">
						  	<c:if test="${e.flagInfoCatalogo == 'S'}">
							  	...
							  	<c:forEach items="${e.elencoDettagliInfo}" var="f" varStatus="i">
								  	<tr class="row_${e.idGruppoInfo} toggle_${e.idGruppoInfo}" style="display: none">
								  		<td></td>
								  		<td><c:out value="${f.descrizione}"></c:out></td>	
								  				
<%-- 								  		<td><m:radio-list disabled="true" name="listaFlagAllegati_${i.index}" id="listaFlagAllegati_${i.index}" list="${listaFlagAllegati}" selectedValue="${f.flagGestioneFile}"  textProperty="descrizione" valueProperty="id"></m:radio-list></td> --%>
										
										<td>
										<m:select viewTitle="true" viewOptionsTitle="true" disabled="true"
										name="idTipoDoc_${i.index}" list="${listaTipiFile}" id="idTipoDoc_${i.index}"  headerValue="-- selezionare --"
										valueProperty="id" textProperty="descrizione" selectedValue="${f.extIdTipoDocumento}" />
										</td>
					...
					<fieldset>
				 		<legend>Allegati aggiuntivi</legend>
						...
						<colgroup>
							...
							<thead>
								<tr> 
									...
									<th>Gruppo allegato e Dettaglio allegato</th>
									<th>Obbligatorio</th>
									<th>Tipo File</th>  
									<th>Vincolo tra gli Allegati</th>
								</tr>
							</thead>
							<tbody> 
								...
								<c:choose>
									...
									<c:forEach items="${nuoveDichiarazioni}" var="a" varStatus="l">
										<tr class="newGroup row_${l.index}"><td colspan="6"><input type="hidden" id="idGruppoInfo_${l.index}" name="idGruppoInfo_${l.index}" value="${a.idGruppoInfo}"><i>Gruppo allegato *</i></td></tr>
										<tr class="newGroup row_${l.index}" >
											<td>
												...
												<c:if test="${(flagAttivoDichiarazione == 'N' && utenteAbilitazioni.dirittoAccessoPrincipale=='W' && !sessionProcedimentoOggettoQuadroInserito) || (requestScope[\"isNuovoInserimento_\".concat(l.index)])}">
													<a onclick="$('.row_${l.index}').remove();return false;" class="ico24 ico_trash" href="elimina_gruppo.do"></a>
												</c:if>
											</td>
											<td colspan="5"><m:textarea readonly="${flagAttivoDichiarazione == 'S' || utenteAbilitazioni.dirittoAccessoPrincipale=='R'}" name="txtAreaGruppo_${l.index}" id="txtAreaGruppo_${l.index}"  style="width:100%">${a.descrizione}</m:textarea></td>
										</tr>
										<tr class="newGroup newDetail detRow_${l.index} "><td colspan="6"><i>Dettaglio allegato * 
										<c:if test="${flagAttivoDichiarazione == 'N'  && utenteAbilitazioni.dirittoAccessoPrincipale=='W'}">
										<a onclick="addNewDettaglioInfo(${l.index});return false;" href="#" class="ico16 ico_add"></a>
										...
										<c:forEach items="${a.elencoDettagliInfo}" var="b" varStatus="c">
											<tr class="newGroup newDetail row_${l.index} detRow_${l.index} detRow_${c.index}_${l.index}"> 
												<td>
													...
													<c:if test="${(flagAttivoDichiarazione == 'N' && utenteAbilitazioni.dirittoAccessoPrincipale=='W' && !sessionProcedimentoOggettoQuadroInserito) || (requestScope[\"isNuovoInserimento_\".concat(c.index).concat('_').concat(l.index)])}">
															<a onclick="$('.detRow_${c.index}_${l.index}').remove();return false;" class="ico24 ico_trash" href="elimina_gruppo.do"></a>
													</c:if>
												</td> 
												<td><m:textarea readonly="${flagAttivoDichiarazione == 'S' || utenteAbilitazioni.dirittoAccessoPrincipale=='R'}" id="txtAreaGruppo_${c.index}_${l.index}" name="txtDettaglioGruppo_${c.index}_${l.index}" style="width:100%">${b.descrizione}</m:textarea></td>
												<td><b:checkBox disabled="${flagAttivoDichiarazione == 'S' || utenteAbilitazioni.dirittoAccessoPrincipale=='R'}" name="flagObblDetailGruppo_${c.index}_${l.index}" value="${c.index}" checked="${b.flagObbligatorio == 'S'}"></b:checkBox></td>
<%-- 												<td><m:radio-list disabled="${flagAttivoDichiarazione == 'S' || utenteAbilitazioni.dirittoAccessoPrincipale=='R'}" onclick="changeTipoDocumento('listaFlagAllegati_${c.index}_${l.index}','${c.index}_${l.index}');" name="listaFlagAllegati_${c.index}_${l.index}" id="listaFlagAllegati_${c.index}_${l.index}" list="${listaFlagAllegati}" selectedValue="${b.flagGestioneFile}" textProperty="descrizione" valueProperty="id"></m:radio-list></td> --%>
												<td>
													<m:select viewTitle="true" viewOptionsTitle="true" disabled="${flagAttivoDichiarazione == 'S' || utenteAbilitazioni.dirittoAccessoPrincipale=='R'}" 
													name="idTipoDocumento_${c.index}_${l.index}" list="${listaTipiFile}" id="idTipoDocumento_${c.index}_${l.index}"  headerValue="-- selezionare il tipo di allegato --"
													valueProperty="id" textProperty="descrizione" selectedValue="${b.extIdTipoDocumento}" />
												</td>
												<td><m:select viewTitle="true" viewOptionsTitle="true" selectedValue="${mapIdLegamiInfo[b.idLegameInfoStr]}" preferRequestValues="${preferRqParameter}" disabled="${flagAttivoDichiarazione == 'S' || utenteAbilitazioni.dirittoAccessoPrincipale=='R'}" id="selvincolo_${c.index}_${l.index}" list="${elencoVincoli}" textProperty="descrizione" valueProperty="id"  name="selvincolo_${c.index}_${l.index}"></m:select></td>
											</tr>
										</c:forEach>
										<tr class="newGroup row_${l.index}"><td colspan="6">&nbsp;</td></tr>
									</c:forEach>
								</c:when>
								<c:when test="${elencoGruppiInfoNoCatalogo != null }">
										<c:forEach items="${elencoGruppiInfoNoCatalogo}" var="a" varStatus="l">
											<c:if test="${a.flagInfoCatalogo == 'N'}">
												<tr class="newGroup row_${l.index}"><td colspan="6"><input type="hidden" id="idGruppoInfo_${l.index}" name="idGruppoInfo_${l.index}" value="${a.idGruppoInfo}"><i>Titolo allegato *</i></td></tr>
												<tr class="newGroup row_${l.index}" >
													<td>
														<c:if test="${a.elencoDettagliInfo != null}">
															<input type="hidden" id="maxDetail_${l.index}" value="${fn:length(a.elencoDettagliInfo)-1}">
													...
													<td colspan="5"><m:textarea readonly="${flagAttivoDichiarazione == 'S'}"  name="txtAreaGruppo_${l.index}" id="txtAreaGruppo_${l.index}"  style="width:100%">${a.descrizione}</m:textarea></td>
 												</tr>
												<tr class="newGroup newDetail row_${l.index} detRow_${l.index} "><td colspan="6"><i>Dettaglio allegato * <c:if test="${flagAttivoDichiarazione == 'N'}"><a onclick="addNewDettaglioInfo(${l.index});return false;" href="#" class="ico16 ico_add" style="margin-left:1em"></a><a style="margin-left:1em" onclick="reorderManager(${l.index});return false;" href="#" class="ico16 ico_up_down"></a></c:if></i></td></tr>
												<c:forEach items="${a.elencoDettagliInfo}" var="b" varStatus="c">
													<tr class="newGroup newDetail row_${l.index} detRow_${l.index} detRow_${c.index}_${l.index}">
														<td> 
															<input type="hidden" id="idDettaglioInfo_${c.index}_${l.index}" name="idDettaglioInfo_${c.index}_${l.index}" value="${b.idDettaglioInfo}">
...
<%-- 														<td><m:radio-list disabled="${flagAttivoDichiarazione == 'S' || utenteAbilitazioni.dirittoAccessoPrincipale=='R'}" onclick="changeTipoDocumento('listaFlagAllegati_${c.index}_${l.index}','${c.index}_${l.index}');" name="listaFlagAllegati_${c.index}_${l.index}" id="listaFlagAllegati_${c.index}_${l.index}" list="${listaFlagAllegati}" selectedValue="${b.flagGestioneFile}" textProperty="descrizione" valueProperty="id"></m:radio-list></td> --%>
								...
								<tr id="hNewGroup" style="display: none">
									<td>
										...
										<a onclick="$('.row_$$index').remove();return false;" class="ico24 ico_trash" href="elimina_gruppo.do"></a>
									</td>
									<td colspan="5"><m:textarea name="txtAreaGruppo_$$index" id="txtAreaGruppo_$$index" style="width:100%"></m:textarea></td>
								</tr>
								<tr id="hNewDetail" style="display: none">
									<td>
										<a onclick="$('.detRow_$$idGruppo_$$index').remove();return false;" class="ico24 ico_trash" href="elimina_gruppo.do"></a>
									</td>
									<td><m:textarea name="txtDettaglioGruppo_$$index_$$idGruppo" id="txtAreaGruppo_$$index_$$idGruppo" style="width:100%"></m:textarea></td>
									<td><b:checkBox name="flagObblDetailGruppo_$$index_$$idGruppo" value="$$index" checked="false"></b:checkBox> </td>
<%-- 									<td><m:radio-list onclick="changeTipoDocumento('listaFlagAllegati_$$index_$$idGruppo','$$index_$$idGruppo');" name="listaFlagAllegati_$$index_$$idGruppo" id="listaFlagAllegati_$$index_$$idGruppo" list="${listaFlagAllegati}" selectedValue="N" textProperty="descrizione" valueProperty="id"></m:radio-list> </td> --%>
									<td><m:select viewTitle="true" viewOptionsTitle="true" name="idTipoDocumento_$$index_$$idGruppo" visible="true" list="${listaTipiFile}" id="idTipoDocumento_$$index_$$idGruppo"  headerValue="-- selezionare il tipo di allegato --" valueProperty="id" textProperty="descrizione" /> </td>
				 ...
				 <div class="form-group puls-group" style="margin-top: 2em">
					<div class="col-sm-12">
						<button type="button" onclick="forwardToPage('impegni.do');" class="btn btn-default">indietro</button>
			    ...
			    <div class="modal fade" id="gruppiCatalogoModal" tabindex="-1" role="dialog" aria-labelledby="gruppiCatalogoModalLabel" aria-hidden="true">  		
					<div class="modal-dialog" style="width: 850px">                                                                                                         			
					  <div class="modal-content">                                                                                                      			
					    <div class="modal-header">                                                                                                     			
					      <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button> 	
					      <h4 class="modal-title" id="myModalLabel">Catalogo Allegati Standard</h4>                                                               		
					    </div>                                                                                                                         				
					    <div class="modal-body">   
							<table id="tGruppo" class="table table-hover table-striped table-bordered tableBlueTh" >
								<colgroup>
...
<script type="text/javascript">
	...
	function changeTipoDocumento(idSelectObbligatorio, row)
	{
		var val = $('input:radio[name='+idSelectObbligatorio+']:checked').val();
		if(val == 'O' || val == 'F'){
			$('#idTipoDocumento_'+row).show();
		}else{
			$('#idTipoDocumento_'+row).val($('#idTipoDocumento_'+row+' option:first').val());
			$('#idTipoDocumento_'+row).hide();
		}
	}
	...
	function confermaGruppo()
	{
		...
		$('#mainForm').attr('action','visualizzaallegati.do').submit();
	}
	function confermaGruppoCatalogo()
	{
		$('#gruppiCatalogoModal').modal('hide');
		$('#mainForm').attr('action','refreshGruppoInfoAllegati.do').submit();
	}
	function addNewGruppoInfo()
	{
		...
		$('#maxid').val(newId);
		$('#tNewGruppo tbody').append('<tr class="newGroup row_'+newId+'"><td colspan="6"><i>Gruppo allegato *</i></td></tr>');
		$('#tNewGruppo tbody').append('<tr class="newGroup row_'+newId+'">'+$('#hNewGroup').clone().html().replace(/\\$\\$index/g, newId)+'</tr>');
		
		maxId = $('#maxDetail_'+newId).val();
		var newIdDet = parseInt(maxId) + 1 ;
		$('#maxDetail_'+newId).val(newIdDet);
		var txt = $('#hNewDetail').clone().html().replace(/\\$\\$index/g, newIdDet);
		txt = txt.replace(/\\$\\$idGruppo/g, newId)
		$('#tNewGruppo tbody').append('<tr class="newGroup newDetail row_'+newId+' detRow_'+newId+'  "><td colspan="6"><i>Dettaglio allegato * <a onclick="addNewDettaglioInfo('+newId+');return false;" href="#" class="ico16 ico_add" href="#"></a>  <a style="margin-left:1em" onclick="reorderManager('+newId+');return false;" href="#" class="ico16 ico_up_down"></a> </i></td></tr>');
		$('#tNewGruppo tbody').append('<tr class="newGroup newDetail row_'+newId+' detRow_'+newId+' detRow_'+newId+'_'+newIdDet+'">'+txt+'</tr>');
		$('#tNewGruppo tbody').append('<tr class="newGroup row_'+newId+'"><td colspan="6">&nbsp;</td></tr>');
	}
	...
	function addNewDettaglioInfo(idGruppo)
	{
		...
		var txt = $('#hNewDetail').clone().html().replace(/\\$\\$index/g, newId);
		
		if(newId <= 0)
			txt = $('#hNewDetailNoTrash').clone().html().replace(/\\$\\$index/g, newId);
	...
	function submitConferma()
	{
		...
		$('#mainForm').attr("action", "confermaallegati.do");
		$('#mainForm').submit();
	...
	{
		var act = $('#mainForm').attr("action");
		$('#mainForm').attr("action", "avantiallegati.do");
		$('#mainForm').submit();
		$('#mainForm').attr("action", act);
	}
	...
	function spostaSu(rowIndex, idGruppo)
	{
		...
		if(newTxt!==undefined){
			...
			var thisFlagAl =$('#listaFlagAllegati_'+rowIndex+'_'+idGruppo+'').val();
			var newFlagAl  =$('#listaFlagAllegati_'+(Number(rowIndex)-1)+'_'+idGruppo+'').val();
			var thisTipoDoc =$('#idTipoDocumento_'+rowIndex+'_'+idGruppo+'').val();
			var newTipoDoc  =$('#idTipoDocumento_'+(Number(rowIndex)-1)+'_'+idGruppo+'').val();

			$('#listaFlagAllegati_'+rowIndex+'_'+idGruppo+'').val(newFlagAl );
			$('#listaFlagAllegati_'+(Number(rowIndex)-1)+'_'+idGruppo+'').val(thisFlagAl );
			$('#idTipoDocumento_'+rowIndex+'_'+idGruppo+'').val(newTipoDoc );
			$('#idTipoDocumento_'+(Number(rowIndex)-1)+'_'+idGruppo+'').val(thisTipoDoc );
