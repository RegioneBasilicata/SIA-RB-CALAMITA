...
@Controller
@RequestMapping("/cunembo263i")
@NemboSecurity(value = "CU-NEMBO-263-I", controllo = NemboSecurity.Controllo.PROCEDIMENTO)
public class CUNEMBO263InserisciDocumentiSpesa
    extends DocumentiSpesaBaseController
{
      ...
      throws InternalUnexpectedException
  {
    ...
    if (errors.isEmpty())
    {
      ...
      BigDecimal sumNEMBO_R_DOC_SPESA_INT_RICEV_PA = quadroEJB
          .getImportoDocSpesaIntRicPag(ricevuta.getIdRicevutaPagamento());
      if (sumNEMBO_R_DOC_SPESA_INT_RICEV_PA != null)
        if (ricevuta.getImportoPagamento()
            .compareTo(sumNEMBO_R_DOC_SPESA_INT_RICEV_PA) < 0)
          errors.addError("importoPagamento",
              "Il valore non deve essere inferiore a quanto già associato agli interventi, "
                  + sumNEMBO_R_DOC_SPESA_INT_RICEV_PA);
