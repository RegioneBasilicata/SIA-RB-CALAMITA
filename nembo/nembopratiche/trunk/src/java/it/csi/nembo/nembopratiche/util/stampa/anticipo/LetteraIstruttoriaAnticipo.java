...
public class LetteraIstruttoriaAnticipo extends Stampa
{
  ...
  public Stampa findStampaFinale(long idProcedimentoOggetto, String cuName) throws Exception
  {
	...
	{
		if(NemboConstants.OGGETTO.CODICE.ISTRUTTORIA_ANTICIPO.equals(procOggDTO.getCodOggetto()))
		{
	...
	if(progressivoIstanzaISACC > 2)
		throw new ApplicationException("Attenzione! si è verificato un errore durante la stampa del Lettera. Contattare l'assistenza tecnica");
    ...
    if (esitoPositivo)
    {
    	if(progressivoIstanzaISACC == 1)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ANTICIPO_POSITIVO_1);
    	else if(progressivoIstanzaISACC == 2)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ANTICIPO_POSITIVO_2);
    }
    else if(NemboConstants.ESITO.TIPO.PARZIALMENTE_POSITIVO.equals(esitoFinaleDTO.getCodiceEsito()))
    {
   		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ANTICIPO_PARZIALE_1);
    }
    else
    {
    	if(progressivoIstanzaISACC == 1)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ANTICIPO_NEGATIVO_1);
    	else if(progressivoIstanzaISACC == 2)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ANTICIPO_NEGATIVO_2);
    }
