          ...
          if (testoRifData.equals(text))
          {
            ...
            if (lastBox != null)
            {
              ...
              contentStream.newLineAtOffset(
                  Float.parseFloat(String.valueOf(lastBox.getP0().getX() + 10)),
                  Float
                      .parseFloat(String.valueOf(lastBox.getP0().getY() + 20)));
          ...
          if (text.startsWith(testoRifFirma))
          {
            ...
            if (lastBox != null)
            {
              ...
              if (offsetFirma == null)
              {
                ...
                contentStream.newLineAtOffset(
                    Float
                        .parseFloat(String.valueOf(lastBox.getP0().getX() + 2)),
                    Float.parseFloat(
                        String.valueOf(lastBox.getP0().getY() + offestFirma)));
              }
              else
              {
                offestFirma = 20 + offsetFirma;
                contentStream.newLineAtOffset(
                    Float
                        .parseFloat(String.valueOf(lastBox.getP0().getX() + 2)),
                    Float.parseFloat(
              ...
              // testoFirma potrebbe contenere &acapo& , in quel caso devo
              // forzare il new line
              if (testoFirma.indexOf(
                  NemboConstants.FIRMA_GRAFOMETRICA.TESTO.A_CAPO) < 0)
              {
                contentStream.showText(testoFirma);
              }
              else
              {
                String[] testi = testoFirma
                    .split(NemboConstants.FIRMA_GRAFOMETRICA.TESTO.A_CAPO);
              ...
              if (!GenericValidator.isBlankOrNull(integrazioneFirma))
              {
                ...
                contentStream.newLineAtOffset(
                    ...
                    Float.parseFloat(String
                        .valueOf(lastBox.getP0().getY() + offestFirma - 15)));
