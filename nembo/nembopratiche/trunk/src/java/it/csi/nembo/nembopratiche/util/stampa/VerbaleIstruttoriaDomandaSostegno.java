...
public class VerbaleIstruttoriaDomandaSostegno extends Stampa
{
  ...
  public Stampa findStampaFinale(long idProcedimentoOggetto, String cuName) throws Exception
  {
	...
	if(progressivoIstanza > 2)
		throw new ApplicationException("Attenzione! si è verificato un errore durante la stampa del Verbale. Contattare l'assistenza tecnica");
    ...
    if (esitoPositivo)
    {
    	if(progressivoIstanza == 1)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_VERBALE_ISTRUTTORIA_DOMANDA_SOSTEGNO_POSITIVO_1);
    	else if(progressivoIstanza == 2)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_VERBALE_ISTRUTTORIA_DOMANDA_SOSTEGNO_POSITIVO_2);
    }
    else if(NemboConstants.ESITO.TIPO.PARZIALMENTE_POSITIVO.equals(esitoFinaleDTO.getCodiceEsito()))
    {
    	if(progressivoIstanza == 1)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_VERBALE_ISTRUTTORIA_DOMANDA_SOSTEGNO_PARZIALE_1);
    	else if(progressivoIstanza == 2)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_VERBALE_ISTRUTTORIA_DOMANDA_SOSTEGNO_PARZIALE_2);
    }
    else
    {
    	if(progressivoIstanza == 1)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_VERBALE_ISTRUTTORIA_DOMANDA_SOSTEGNO_NEGATIVO_1);
    	else if(progressivoIstanza == 2)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_VERBALE_ISTRUTTORIA_DOMANDA_SOSTEGNO_NEGATIVO_2);
    }
