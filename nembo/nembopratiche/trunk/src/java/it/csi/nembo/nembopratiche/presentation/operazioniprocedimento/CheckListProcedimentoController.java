...
@Controller
@NemboSecurity(value = "CU-NEMBO-270", controllo = NemboSecurity.Controllo.PROCEDIMENTO_OGGETTO)
public class CheckListProcedimentoController extends BaseController
{
  ...
  @RequestMapping(value = "/cunembo270/index", method = RequestMethod.POST)
  @IsPopup
  @ResponseBody
      ...
      throws InternalUnexpectedException
  {
    ...
    sb.append(
        "\n<div class=\"row\"><a style=\"margin-bottom:2em\" class=\"pull-right ico_spaced ico24 ico_print\" href=\"../cunembo274/stampa.do\" title=\"Stampa\"></a></div>");
  ...
  {
    if (codice.equals(NemboConstants.OGGETTO.CODICE.DOMANDA_SOSTEGNO)
        || codice
            .equals(NemboConstants.OGGETTO.CODICE.DOMANDA_SOSTEGNO_PAGAMENTO)
        || codice
            .equals(NemboConstants.OGGETTO.CODICE.DOMANDA_SOSTEGNO_PREMIO))
    {
      ...
      if (codice.equals(
          NemboConstants.OGGETTO.CODICE.ISTRUTTORIA_AMMISSIONE_FINANZIAMENTO)
          || codice.equals(
          ...
          || codice.equals(
              NemboConstants.OGGETTO.CODICE.ISTRUTTORIA_AMMISSIBILITA_FINANZIAMENTO))
      {
        writeIstruttoriaInvestimento(sb, oggetto);
      }
      else
        if (codice.equals(NemboConstants.OGGETTO.CODICE.DOMANDA_SALDO)
            || codice.equals(NemboConstants.OGGETTO.CODICE.DOMANDA_ACCONTO)
            || codice.equals(NemboConstants.OGGETTO.CODICE.DOMANDA_ANTICIPO))
        {
          writeDomandaPagamento(sb, oggetto);
        }
        else
          if (codice
              .equals(NemboConstants.OGGETTO.CODICE.ISTRUTTORIA_ACCONTO)
              || codice
                  .equals(NemboConstants.OGGETTO.CODICE.ISTRUTTORIA_SALDO)
              || codice.equals(
                  NemboConstants.OGGETTO.CODICE.ISTRUTTORIA_ANTICIPO))
          {
      ...
      throws InternalUnexpectedException
  {
    ...
    List<ControlloAmministrativoDTO> controlliAmministrativi = quadroEJB
        .getControlliAmministrativi(oggetto.getIdProcedimentoOggetto(),
            NemboConstants.QUADRO.CODICE.CONTROLLI_AMMINISTRATIVI, null);
      ...
      if (esitoFinale != null)
      {
        ...
        sb.append(
            "<div class=\"panel panel-primary custom-collapsible-panel\" id=\"esitoFinale\">"
                ...
                + NemboUtils.STRING.nvl(StringEscapeUtils
                ...
                + StringEscapeUtils.escapeHtml4(
                    NemboUtils.STRING.nvl(esitoFinale.getDescrTecnico()))
                + "</div>"
                + "</div>"
                + "<div class=\"row\">"
        ...
        if (interventi != null)
        {
          ...
          for (RigaJSONInterventoQuadroEconomicoByLivelloDTO item : interventi)
          {
            ...
            sb.append("<tr>"
                ...
                + "<td>"
                + NemboUtils.DATE.formatDate(item.getDataAmmissione())
                + "</td>"
                + "<td>&euro; " + NemboUtils.FORMAT
                    .formatCurrency(item.getImportoInvestimento())
                + "</td>"
                + "<td>&euro; "
                + NemboUtils.FORMAT.formatCurrency(item.getImportoAmmesso())
                + "</td>"
                + "<td>&euro; " + NemboUtils.FORMAT
      ...
      throws InternalUnexpectedException
  {
    ...
    if (visite != null)
    {
      ...
      {
        sb.append("<tr>"
            + "<td>" + NemboUtils.DATE.formatDate(item.getDataVisita())
            + "</td>"
            + "<td>" + StringEscapeUtils.escapeHtml4(
                NemboUtils.STRING.nvl(item.getDescTecnico()))
            + "</td>"
            + "<td>" + NemboUtils.STRING.nvl(item.getDescEsito()) + "</td>"
            + "<td>" + NemboUtils.DATE.formatDate(item.getDataVerbale())
            + "</td>"
            + "<td>" + NemboUtils.STRING.nvl(item.getNumeroVerbale())
            + "</td>"
            + "</tr>");
      }
      sb.append("</tbody></table>");
      ...
      throws InternalUnexpectedException
  {
    ...
    QuadroOggettoDTO quadro = procedimentoOggetto.findQuadroByCodiceQuadro(
        NemboConstants.QUADRO.CODICE.DATI_IDENTIFICATIVI);
      ...
      throws InternalUnexpectedException
  {
	  ...
	  if((NemboConstants.OGGETTO.CODICE.DOMANDA_SALDO).equals(oggetto.getCodice())){
		  datiProt =ricercaEJB.findProtocolloPraticaByIdPOAndCodice(oggetto.getIdProcedimentoOggetto(), 2007);
	  }else if((NemboConstants.OGGETTO.CODICE.DOMANDA_ACCONTO).equals(oggetto.getCodice())){
		  datiProt =ricercaEJB.findProtocolloPraticaByIdPOAndCodice(oggetto.getIdProcedimentoOggetto(), 2006);
	  }else if((NemboConstants.OGGETTO.CODICE.DOMANDA_ANTICIPO).equals(oggetto.getCodice())){
    ...
    sb.append(
        "<div class=\"panel panel-primary custom-collapsible-panel\" id=\"domandaPagamento\">"
            ...
            + "<div class=\"row\">"
            + "<div class=\"col-md-3\"><label >Data Protocollo pratica</label></div><div class=\"col-md-9\">"
            + NemboUtils.STRING.nvl(datiProt.getDescrizione()) + "</div>"
            + "</div>"
            + "<div class=\"row\">"
            + "<div class=\"col-md-3\"><label >Organismo delegato</label></div><div class=\"col-md-9\">"
            + NemboUtils.STRING.nvl(
      ...
      for (ControlloAmministrativoDTO controllo : controlliAmministrativi)
      {
        if (controllo.getIdControlloAmministratPadre() != null)
        {
          sb.append("<div class=\"row\">"
              ...
              + NemboUtils.STRING.nvl(controllo.getDescEsito()) + "</div>"
              + "</div>");
        }
        else
        {
    ...
    if (datiSpecifici != null)
    {
      sb.append(
          "<div class=\"panel panel-primary custom-collapsible-panel\" id=\"estrazione\">"
              ...
              + NemboUtils.DATE.formatDate(datiSpecifici.getDataEstrazione())
              + "</div>"
              + "</div>"
              + "<div class=\"row\">"
    ...
    if (importi != null)
    {
      ...
      for (ImportoLiquidatoDTO item : importi)
      {
        ...
        sb.append("<tr>"
            + "<td>" + item.getCodiceLivello() + "</td>"
            + "<td>&euro; "
            + NemboUtils.FORMAT.formatCurrency(item.getImportoLiquidato())
            + "</td>"
        ...
        if (quote != null)
        {
          ...
          for (RipartizioneImportoDTO itemq : quote)
          {
            ...
            sb.append("<tr>"
                ...
                + "<td>" + NemboUtils.FORMAT.formatCurrency(
