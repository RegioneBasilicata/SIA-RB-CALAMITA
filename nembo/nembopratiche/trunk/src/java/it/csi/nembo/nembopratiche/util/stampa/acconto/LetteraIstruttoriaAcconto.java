  ...
  public Stampa findStampaFinale(long idProcedimentoOggetto, String cuName) throws Exception
  {
    ...
    if (esitoPositivo)
    {
    	if(progressivoIstanzaISACC == 1)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ACCONTO_POSITIVO_1);
    	else if(progressivoIstanzaISACC == 2)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ACCONTO_POSITIVO_2);
    }
    else if(NemboConstants.ESITO.TIPO.PARZIALMENTE_POSITIVO.equals(esitoFinaleDTO.getCodiceEsito()))
    {
   		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ACCONTO_PARZIALE_1);
    }
    else
    {
    	if(progressivoIstanzaISACC == 1)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ACCONTO_NEGATIVO_1);
    	else if(progressivoIstanzaISACC == 2)
    		return NemboUtils.STAMPA.getStampaFromCdU(NemboConstants.USECASE.STAMPE_OGGETTO.GENERA_LETTERA_ISTRUTTORIA_ACCONTO_NEGATIVO_2);
    }
